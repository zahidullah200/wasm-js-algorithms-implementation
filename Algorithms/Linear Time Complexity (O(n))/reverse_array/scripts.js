...
function reverseJsArray(arr) {
  ...
  while (start < end) {
   ...
   for ( i = 0; i < 200000; i++) {
    ...
    let dummy = arr[i];
    dummy+=dummy*2; // Dummy operation to prevent optimization
   }
  return arr;
}
...
function reverseArr(inputArray, outputDiv, performanceDiv) {
  return new Promise((resolve, reject) => {
    ...
    if (
      ...
      const newMemorySize =
        Math.ceil(
          (arrayPointer + arrayLength * Int32Array.BYTES_PER_ELEMENT) / 131072) * 131072; // Adjust memory size in 128KB increments
      try {
        Module.memory.grow(
